...
int byteWeight(unsigned char b) {
	int weight = 0;	
	for (int i = 0; i < 8; i++) {
		weight += (b >> i)&0x01;
	}
...
{
    int l = ((struct Character *)p)->weight;
    int r = ((struct Character *)q)->weight; 
    return (l - r);
}

int main(int argc, char *argv[]) {
	struct Character chars[0x60];
	for(char i = 0x0; i < 0x60; i++) {
		int weight = 0;
		for (int j = 0; j < 16; j++) {
			weight += byteWeight(nAsciiDot[i*16 + j]);
		}
		chars[i].chr = i + 0x20;
		chars[i].weight = weight;
	}
	qsort((void*)chars, 0x60, sizeof(chars[0]), comparator);
	for(char i = 0x0; i < 0x60; i++) {
		printf("%c: %d\n", chars[i].chr, chars[i].weight);
	}
}
